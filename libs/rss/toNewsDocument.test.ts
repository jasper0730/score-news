...
describe('buildArticleId', () => {
    ...
    it('不同媒體即使 guid 相同也不會撞號', () => {
        ...
        expect(buildArticleId('cna', item)).not.toBe(buildArticleId('ltn', item))
    })

    it('沒有 guid 時改用連結，結果一樣穩定', () => {
        const item = makeItem({ guid: null })

        expect(buildArticleId('ltn', item)).toBe(buildArticleId('ltn', item))
    })

    it('沒有 guid 時，連結不同就是不同文章', () => {
        const a = buildArticleId('ltn', makeItem({ guid: null, link: 'https://a.com/1' }))
        const b = buildArticleId('ltn', makeItem({ guid: null, link: 'https://a.com/2' }))
