...
| 自由時報 LTN   |      11 |      440 | 需抓 og:image            | **無 → 用連結雜湊**           |
...
**合計 43 個 feed、單次約 1743 則，去重後 1654 篇。**
...
```ts
export function buildArticleId(outlet: string, item: { guid; link }): string {
...
1. **只用 guid／link 當種子**，不摻時間、亂數、抓取順序 → 保證可重現
2. **不用標題當種子** → 媒體改標題時不會變成「新文章」
3. **加上 outlet 前綴** → 不同媒體剛好用同一組 guid 也不會撞號

自由時報是唯一沒有 guid 的來源，退回用文章連結。

> ⚠️ **追問：「那如果媒體改了文章網址呢？」**
> 老實說會斷。自由時報這一家有這個風險，其他六家有 guid 所以不受影響。
> 這是已知限制，我在文件裡標記為「規則一旦上線就不能改」。
...
只有中央社、自由時報、公視需要額外抓頁面。首次匯入 685 次請求，
**比「每篇都抓」少一半以上**。
...
新聞頁動輒數百 KB，為了一個 meta 標籤拉完整頁，685 篇就是上百 MB 的無謂流量。
